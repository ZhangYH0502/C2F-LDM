...
class MsImageDis(nn.Module):
    ...
    def __init__(self, input_dim, params):
        ...
        self.downsample = nn.AvgPool3d(3, stride=(2,2,2), padding=1, count_include_pad=False)
    ...
    def _make_net(self):
        dim = self.dim
        cnn_x = []
        cnn_x += [Conv3dBlock(self.input_dim, dim, (4,4,4), (2,2,2), 1, norm='none', activation=self.activ, pad_type=self.pad_type)]
        for i in range(self.n_layer - 1):
            cnn_x += [Conv3dBlock(dim, dim * 2, (4,4,4), (2,2,2), 1, norm=self.norm, activation=self.activ, pad_type=self.pad_type)]
            dim *= 2
...
class VAEGen(nn.Module):
    # VAE architecture
    def __init__(self, input_dim, params, use_noise):
...
class Decoder(nn.Module):
    def __init__(self, n_upsample, n_res, dim, output_dim, res_norm='adain', activ='relu', pad_type='zero'):
        ...
        for i in range(n_upsample):
            self.model += [nn.Upsample(scale_factor=2, mode='trilinear', align_corners=True),
...
class ResBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out += residual
        return out
class CustomPad3D(torch.nn.Module):
    def __init__(self, padding, pad_type):
        super(CustomPad3D, self).__init__()
        self.padding = (padding,padding,padding,padding,padding,padding)
        self.pad_type = pad_type
    def forward(self, x):
        return F.pad(x, self.padding, mode=self.pad_type)
...
class Vgg16(nn.Module):
    def __init__(self):
        ...
        self.conv1_1 = nn.Conv3d(3, 64, kernel_size=3, stride=1, padding=1)
        self.conv1_2 = nn.Conv3d(64, 64, kernel_size=3, stride=1, padding=1)

        self.conv2_1 = nn.Conv3d(64, 128, kernel_size=3, stride=1, padding=1)
        self.conv2_2 = nn.Conv3d(128, 128, kernel_size=3, stride=1, padding=1)

        self.conv3_1 = nn.Conv3d(128, 256, kernel_size=3, stride=1, padding=1)
        self.conv3_2 = nn.Conv3d(256, 256, kernel_size=3, stride=1, padding=1)
        self.conv3_3 = nn.Conv3d(256, 256, kernel_size=3, stride=1, padding=1)

        self.conv4_1 = nn.Conv3d(256, 512, kernel_size=3, stride=1, padding=1)
        self.conv4_2 = nn.Conv3d(512, 512, kernel_size=3, stride=1, padding=1)
        self.conv4_3 = nn.Conv3d(512, 512, kernel_size=3, stride=1, padding=1)

        self.conv5_1 = nn.Conv3d(512, 512, kernel_size=3, stride=1, padding=1)
        self.conv5_2 = nn.Conv3d(512, 512, kernel_size=3, stride=1, padding=1)
        self.conv5_3 = nn.Conv3d(512, 512, kernel_size=3, stride=1, padding=1)
...
class depthwise_separable_conv(nn.Module):
    def __init__(self, nin, nout, kernel_size, padding):
        ...
        self.depthwise = nn.Conv3d(nin, nin, kernel_size=kernel_size, padding=padding, groups=nin)
